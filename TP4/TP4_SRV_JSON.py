...
class Serveur:
    ...
    def communication_client(self, c):
        nom = None
        try:
            c.send("Connectez-vous en envoyant votre JSON.".encode())  # ✅ Message corrigé

            # ✅ Parsing JSON à la place de ET.fromstring()
            data_json = c.recv(1024).decode().strip()
            client_info = json.loads(data_json)
            nom            = client_info["nom"]
            lieu           = client_info["lieu"]
            date_connexion = client_info["date_connexion"]
        ...
        except json.JSONDecodeError:  # ✅ Erreur JSON à la place de ET.ParseError
            print("Erreur : JSON invalide reçu à la connexion.")
            c.send("Erreur : JSON invalide.".encode())
